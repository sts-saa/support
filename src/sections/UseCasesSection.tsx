...
const useCases = [
  {
    ...
    title: "IT-поддержка",
    description: "Сотрудники обращаются через Telegram или код на рабочем месте. Система сама направляет нужным специалистам.",
    example: {
      title: "Кейс: IT-отдел обрабатывает на 60% больше заявок",
      description: "Сотрудники пишут в корпоративный Telegram-бот. Система определяет тип проблемы и направляет системному администратору, программисту или в службу закупок.",
      features: [
        "Автоматическое определение типа проблемы: оборудование, программы, доступы",
        "Подключение к корпоративным системам",
        "База готовых ответов для частых вопросов",
  ...
  },
  {
    ...
    description: "Участники получают помощь через код на бейдже. Координация помощников, навигация, техподдержка — всё в одном чате.",
    example: {
      title: "Кейс: конференция на 5000 участников без хаоса",
      description: "Код на бейдже каждого участника ведёт в персональный чат поддержки. Система знает программу, местоположение участника и может сразу помочь с навигацией.",
      features: [
  ...
  },
  {
    ...
    description: "Коды в номерах для мгновенного обращения. Гости решают проблемы не выходя из номера, персонал видит точное местоположение.",
    example: {
      title: "Кейс: сеть отелей увеличила оценки гостей на 40%",
      description: "В каждом номере отеля размещен уникальный код. Гость сканирует код и попадает в чат с консьержем. Система автоматически определяет номер комнаты и тип запроса.",
      features: [
  ...
  },
  {
    ...
    description: "Покупатели обращаются прямо из карточки заказа. Система подтягивает историю покупок и помогает с возвратами.",
    example: {
  ...
  },
  {
    ...
    description: "Жильцы подают заявки через код на подъезде. Автоматическое создание заданий, контроль выполнения, массовые уведомления.",
    example: {
      title: "Кейс: УК сократила время реакции в 3 раза",
      description: "Жильцы сканируют код в подъезде и сразу попадают в чат с диспетчером. Система знает адрес, квартиру и может сразу создать задание на работу.",
      features: [
